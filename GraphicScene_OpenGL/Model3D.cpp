	...
	for (size_t s = 0; s < shapes.size(); s++) {
		...
		if (shapes[s].mesh.material_ids.size() > 0) {
			...
			if (currentMatID != -1) {
				...
				std::string matName = materials[currentMatID].name;
				std::string texName = materials[currentMatID].diffuse_texname;
				if (matName.find("Twigs") != std::string::npos || matName.find("Roof") != std::string::npos || matName.find("005") != std::string::npos) {
					std::cout << "!!! DEBUG !!! Shape: " << shapes[s].name << " uses Material: " << matName << std::endl;
					std::cout << "   -> Texture Path in MTL: '" << texName << "'" << std::endl;
				}
			}
		}
		// --- DEBUG END ---
